...
Every WebP meal image — 26 in `public/meals/` in the source project, and the
same 26 in `images/` that the offline build loads — was **generated with an AI
...
Both folders hold the same 26 files: `npm run build` regenerates the offline
...
Twenty-three of the twenty-six recipes are this project's own work, owing nothing
...
the Blackstone garlic-butter vegetables and the loaded bacon breakfast burritos
were supplied by the project owner; the vegetables cite the FDA Food Code,
because vegetables have no USDA minimum. Each recipe's own source list is kept beside it in
