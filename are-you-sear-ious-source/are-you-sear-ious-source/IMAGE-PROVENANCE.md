...
All 21 food images used by this app were generated with an AI image model.
...
- All 21 original PNGs and all 21 delivered WebPs were hashed with SHA-256.
...
- Each original was re-encoded in memory with the existing conversion settings.
  All 21 results matched the delivered WebP files byte for byte. Six supplied
  images have now arrived at 1120x747 rather than the required 1200x800 — two
  on 2026-09-08, and the two chicken plus the burger and lobster on 2026-09-11
  — so their delivered WebPs were re-derived here from the verified originals
  using the pipeline above. `image-provenance.json` records which, and what
...
12 of the 21 PNGs under `originals/`. Every original delivered since then is
