...
type-aware pass, which now runs locally**) · 63 tests pass · build passes · 0
...
3.13 MB across 24 files, but only one is ever loaded.
...
Went from 12 recipes and 1.63 MB to 24 recipes and 727 KB — smaller with twelve
...
**This one has got sharper.** There are now 24 photos averaging 134 KB, and a
...
mobile payload substantially — the photos are 3.13 MB of the 4.0 MB deploy, so
