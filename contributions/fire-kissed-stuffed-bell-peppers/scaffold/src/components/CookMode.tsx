...
export function CookMode({
  ...
  return (
    ...
    >
      ...
      <p>
        {sizeLabel(settings)} · {cuts.find((cut) => cut.id === settings.cut)!.name} ·{' '}
        {target.targetName}
      </p>
      <p className="cook-targets">
        Ambient {temp(recipe.grillAmbientF, settings.tempUnit)} · Finish{' '}
        {temp(target.internalF, settings.tempUnit)} on the grill
      </p>
      <p className="note">
        Safety: {safetySentence(settings)} Pepper tenderness and color are secondary cues, not
      ...
      {done ? (
        <section>
          ...
          <p>
            The checklist cannot confirm doneness. Verify every selected filling reached its target.
          </p>
          <button
            type="button"
            className="secondary"
      ...
      <section className="timer-panel" aria-label="Check-in timer">
        ...
        <p className="note">
          ...
          page. Changing pepper count, style or filling resets this cook session.
        </p>
      </section>
    </dialog>
  );
