...
export function burnerMessage(states: BurnerStates) {
  ...
  if (on === 3)
    return 'All three burners are ON. Turn enough burners OFF to fit every stuffed pepper above an indirect zone.';
  return 'Keep every stuffed pepper above your chosen OFF burners. Adjust the selected ON burners to hold the grate-level ambient target.';
}
